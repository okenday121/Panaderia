              ...
              <form action="{{route('productos.update', $producto)}}" method="POST">
                ...
                <div class="flex items-center mb-5">
                    ...
                    <label for="precio" class="inline-block w-20 mr-6 text-right 
                                             font-bold text-gray-600">Precio Bs.</label>
                    <input value="{{old('precio', $producto->precio)}}" type="text" name="precio"
                           class="flex-1 py-2 border-b-2 border-gray-400 focus:border-indigo-400 
                                  text-gray-600 placeholder-gray-400
                                  outline-none">
                </div>
                ...
                <div class="flex items-center mb-5">
                    <!--         tip - here neede inline-block , but why? -->
                    <label for="ingredientes" class="inline-block w-20 mr-6 text-right 
                                             font-bold text-gray-600">Ingredientes</label>
                    <select name="ingredientes" class="flex-1 py-2 border-b-2 border-gray-400 focus:border-green-400 
                                  text-gray-600 placeholder-gray-400
                                  outline-none">                            
                        @foreach ($ingredientes as $ingrediente)
                            <option value="{{$ingrediente->id}}">{{$ingrediente->detalle}}</option>                                                
                        @endforeach
                    </select>
                </div>
